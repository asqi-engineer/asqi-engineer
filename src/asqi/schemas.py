...
def _validate_s3_bucket(value: str) -> str:
    """Validate S3 bucket rules that are clearer as imperative checks.

    The ``StringConstraints`` regex handles length, allowed characters, and
    leading/trailing alphanumeric characters. These checks cover AWS rules that
    are awkward to encode in Pydantic's Rust regex engine, including IPv4-like
    names, adjacent periods, period/hyphen adjacency, and reserved prefixes or
    suffixes.
    """
    parts = value.split(".")
    if len(parts) == 4 and all(part.isdigit() for part in parts):
        raise ValueError("S3 bucket name must not be formatted as an IPv4 address")
    if ".." in value:
        raise ValueError("S3 bucket name must not contain adjacent periods")
    if ".-" in value or "-." in value:
        raise ValueError("S3 bucket name must not contain periods adjacent to hyphens")
    if value.startswith(("xn--", "sthree-", "amzn-s3-demo-")):
        raise ValueError("S3 bucket name must not use an AWS-reserved prefix")
    if value.endswith(("-s3alias", "--ol-s3", ".mrap", "--x-s3", "--table-s3")):
        raise ValueError("S3 bucket name must not use an AWS-reserved suffix")
    return value


def _validate_s3_key(value: str) -> str:
    """Validate S3 object key path rules for sidecar artifact references.

    S3 permits a broad UTF-8 key space, but the sidecar contract treats keys as
    relative path-like artifact locations. These checks reject absolute paths,
    empty segments, period-only segments, and control characters that AWS
    documents as problematic for object-key consumers. We intentionally do not
    reject every "character to avoid" from the AWS guide because many are valid
    S3 keys and may already appear in customer artifact paths.
    """
    if value.startswith("/"):
        raise ValueError("S3 object key must be relative and must not start with '/'")
    if "//" in value:
        raise ValueError("S3 object key must not contain empty path segments")
    if any(0x00 <= ord(char) <= 0x1F or ord(char) == 0x7F for char in value):
        raise ValueError("S3 object key must not contain control characters (0x00-0x1F, 0x7F)")
    for segment in value.split("/"):
        if segment in (".", ".."):
            raise ValueError("S3 object key must not contain period-only path segments")
    return value
...
S3BucketName = Annotated[
    ...
    AfterValidator(_validate_s3_bucket),
]
S3ObjectKey = Annotated[
    str,
    StringConstraints(min_length=1, strip_whitespace=True),
    AfterValidator(_validate_s3_key),
]


class InputRef(BaseModel):
    """Reference to a single input artifact stored in S3 for sidecar-mediated fetches."""

    bucket: S3BucketName = Field(
        ...,
        description="S3 bucket containing the input artifact.",
    )
    key: S3ObjectKey = Field(
        ...,
        description="S3 object key for the input artifact.",
    )
    checksum: str | None = Field(
        default=None,
        description="Optional checksum for validating the fetched input artifact.",
    )


class OutputDestination(BaseModel):
    """Destination prefix in S3 where the sidecar publishes workload outputs."""

    bucket: S3BucketName = Field(
        ...,
        description="S3 bucket where outputs should be published.",
    )
    key_prefix: S3ObjectKey = Field(
        ...,
        description="S3 key prefix under which output artifacts should be published.",
    )
