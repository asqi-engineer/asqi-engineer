...
from asqi.backends import ContainerBackend, create_backend
...
# OTEL API only: the SDK/providers are installed by the host process (the
# asqi-runner service sets them from OTEL_EXPORTER_OTLP_ENDPOINT). When no
# provider is installed these resolve to no-ops, so standalone CLI runs are
# unaffected. Instrumenting the backend-agnostic execution chokepoint here
# covers every SDG/test container without adding OTEL to each image.
_tracer = trace.get_tracer("asqi.workflow")
_container_meter = metrics.get_meter("asqi.workflow")
_container_duration = _container_meter.create_histogram(
    "asqi.container.duration",
    unit="ms",
    description="Wall-clock duration of a single test/generation container run.",
)
_container_errors = _container_meter.create_counter(
    # Not "asqi.container.errors.total": the OTLP->Prometheus pipeline
    # appends "_total" to counters itself, so a name already ending in
    # ".total" would double-suffix on export (asqi_container_errors_total_total).
    "asqi.container.errors",
    description="Count of failed test/generation container runs, by container type.",
)


def _deployment_environment() -> str | None:
    """Best-effort ``deployment.environment.name`` from OTEL_RESOURCE_ATTRIBUTES.

    The asqi-runner resource carries this attribute (per the OTEL ADR). We
    surface it as an explicit metric/span dimension so container telemetry can
    be sliced by environment even where resource attributes are not promoted to
    Prometheus labels. Returns None when unset (local / CI)."""
...
def _container_span_attributes(metric_type: str, item_name: str) -> dict[str, Any]:
    """AIP-2890 attributes for the asqi.container.run span only.

    Includes asqi.container.name (item_name), which can embed run-unique IDs
    or timestamps for data-generation containers. That's fine on a span, but
    must not flow into the RED metrics below (see _container_metric_attributes).
    """
    attrs: dict[str, Any] = {
        "asqi.container.name": item_name,
        "asqi.metric.type": metric_type,
    }
...
def _container_metric_attributes(metric_type: str) -> dict[str, Any]:
    """Low-cardinality attributes for the asqi.container RED metrics.

    Deliberately excludes asqi.container.name (item_name): it becomes a
    Prometheus label, and item_name can embed run-unique IDs or timestamps
    for data-generation containers, which would blow up Mimir series
    cardinality. Keep that detail on the span only.
    """
    attrs: dict[str, Any] = {"asqi.metric.type": metric_type}
    env = _deployment_environment()
    if env:
        attrs["deployment.environment.name"] = env
    return attrs
...
console = Console()
# Backends are lazily initialized on first use via create_backend() so that
# simply importing this module does not select a backend or require optional
# dependencies (e.g. the kubernetes client when RUN_BACKEND=k8s).
_container_backend: ContainerBackend | None = None
_manifest_backend: ContainerBackend | None = None


def set_container_backend(backend: ContainerBackend) -> None:
    global _container_backend
    _container_backend = backend


def set_manifest_backend(backend: ContainerBackend) -> None:
    global _manifest_backend
    _manifest_backend = backend


def _get_container_backend() -> ContainerBackend:
    global _container_backend
    if _container_backend is None:
        _container_backend = create_backend()
    return _container_backend


def _get_manifest_backend() -> ContainerBackend:
    global _manifest_backend
    if _manifest_backend is None:
        _manifest_backend = create_backend()
    return _manifest_backend
...
def _load_and_merge_environment_variables(
    ...
    item_env_file: str | None,
    item_environment: dict[str, str] | None,
) -> dict[str, str]:
    """
...
def _configure_docker_in_docker(
    ...
    container_env.pop("DOCKER_HOST", None)
...
def _execute_container_job(
    ...
    Args:
        ...
        metric_type: Container type for telemetry ("test" or "generation").
    ...
    _container_start = time.perf_counter()
    _span_attrs = _container_span_attributes(metric_type, item_name)
    _metric_attrs = _container_metric_attributes(metric_type)
    with _tracer.start_as_current_span(
        ...
        try:
            ...
            _span.set_attribute("exit_code", exit_code if exit_code is not None else -1)
            success = container_result.get("success", exit_code == 0)
            if success is False or exit_code not in (0, None):
                container_status = "failed"
                _span.set_status(trace.Status(trace.StatusCode.ERROR, f"container exit code {exit_code}"))
        except Exception as exc:
            container_status = "failed"
            _span.set_status(trace.Status(trace.StatusCode.ERROR, str(exc)))
            _span.record_exception(exc)
            raise
        finally:
            _container_duration.record(
                (time.perf_counter() - _container_start) * 1000.0,
                attributes={**_metric_attrs, "status": container_status},
            )
            if container_status == "failed":
                _container_errors.add(1, attributes=_metric_attrs)
...
def dbos_check_images_availability(images: list[str]) -> dict[str, bool]:
    ...
    return _get_container_backend().check_images(images)


@DBOS.step()
def dbos_pull_images(images: list[str]):
    """Pull missing Docker images from registries."""
    return _get_container_backend().pull_images(images)
...
def evaluate_score_card(
    ...
    for score_card in score_cards:
        try:
            ...
            DBOS.logger.info(
                f"Evaluated score card '{score_card.score_card_name}' "
                f"with {len(score_card_evaluations)} individual evaluations"
            )
...
def validate_test_container_reports(
    ...
    """
    Validates that the reports returned by the test container exactly match the test container
    manifest `output_reports` definitions.
    ...
    for result in all_results:
        ...
        if not result_errors:
            if result.image not in manifests:
                DBOS.logger.warning(
                    f"No manifest found for image '{result.image}', "
                    f"skipping 'generated_reports' validation for test {result.test_id}"
                )
            else:
                ...
                if missing_reports or extra_reports:
                    ...
                    result_errors.append(
                        f"Mismatch between manifest and returned 'generated_reports' for image '{result.image}'. "
                        f"Missing expected reports: {missing_reports}. Extra reports: {extra_reports}."
                    )
...
def validate_display_reports(
    ...
    """
    Validate that score card 'display_reports' are defined in the test container manifests
...
def execute_data_generation(
    ...
    Args:
        ...
        metadata_config: Optional dictionary containing metadata like user_id and job_id
            to forward into the generation container
        parent_id: Optional parent workflow ID for tracking hierarchy (defaults to DBOS.workflow_id)
...
def run_data_generation_workflow(
    ...
    Args:
        ...
        datasets_config: Optional datasets configuration for resolving dataset references
        metadata_config: Optional dictionary containing metadata like user_id and job_id
            to forward into generation containers
