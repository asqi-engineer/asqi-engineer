...
# Implementing Technical Reports in Custom Test Containers

This section describes how to implement technical report output in your custom test containers for ASQI. Follow these guidelines to ensure your container outputs are compatible with the ASQI workflow and score card evaluation system.

## 1. Output Structure

Your container **MUST** output a JSON object to stdout. The JSON object should include:

- `test_results`: (dict) The main results of the test (output_metrics)
- `technical_reports`: (list) List that describes a technical report file generated by the test.
...
## 2. Required Fields for Each Report

Each report generated in `technical_reports` **MUST** return:
...
## 4. Define Reports in Manifest

For every report your container generates, you must add an entry to the `output_reports` list in your manifest. This tells the system exactly what files to expect
...
When running a test in `ASQI Engineer` and once the container has finished executing, the system automatically checks the results against the manifest.
...
- If your container does not output `technical_reports` and `test_results`, ASQI will treat it as legacy output and skip report validation.
- For new containers, always include the `technical_reports` and `test_results` field.