...
def main():
    ...
    try:
        ...
        if response_format == "b64_json":
            ...
            # Mock URL response
            image_data = {"url": f"https://example.com/generated_image_{random.randint(1000, 9999)}.png"}
        ...
        # Validate image generation response format using ASQI validation schema
        try:
            validated_response = validate_image_response(mock_response)

            # Validation succeeded - extract image metrics
            ...
            result = {
                "success": False,
                "score": 0.0,
                "error": f"Image response validation failed: {str(e)}",
                "validation": "failed",
                "base_url": base_url,
                "model": model,
                "prompt": prompt,
