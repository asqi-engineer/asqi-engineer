...
# ── OTEL env propagation (AIP-2890) ────────────────────────────────────────────


class TestOtelEnvInjection:
    """OTLP wiring is passed to BOTH the workload and sidecar containers when
...
class TestKubernetesBackendRun:
    def test_collect_pod_logs_reads_raw_response_without_deserializing_json(
        self,
    ) -> None:
        core_api = MagicMock()
        pod = MagicMock()
