...
class TestEnvironmentVariables:
    ...
    @pytest.fixture
    def sample_embedding_api_systems_config(self):
        """Sample embedding api systems configuration with API key."""

        return SystemsConfig(
            systems={
                "test_system": EmbeddingAPIConfig(
                    type="embedding_api",
                    description="Embedding System description",
                    provider="openai",
                    params=LLMAPIParams(
    ...
    def test_create_test_execution_plan_flattens_embedding_api_system_params(
        self, sample_suite_config, sample_embedding_api_systems_config
    ):
        """Test that create_test_execution_plan correctly flattens Embedding API system parameters."""
        image_availability = {"my-registry/test:latest": True}

        execution_plan = create_test_execution_plan(
            sample_suite_config, sample_embedding_api_systems_config, image_availability
        )
        ...
        # Verify the system params are flattened correctly
        assert system_params["type"] == "embedding_api"
        assert system_params["description"] == "Embedding System description"
        assert system_params["provider"] == "openai"
        assert system_params["base_url"] == "http://EMBEDDING-URL"
        assert system_params["env_file"] == "ENV_FILE"
        assert system_params["model"] == "text-embedding-3-small"
        assert system_params["api_key"] == "sk-embedding-123"
